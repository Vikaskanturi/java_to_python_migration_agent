...
name: doc-agent
description: >
  Use this skill when implementing or running Agent 4 of the Java→Python
  pipeline: the Documentation Agent. This agent reads all state files and
  produces human-readable Markdown documents and a single combined HTML
  report. Triggers on any mention of "documentation agent", "generate docs",
  "migration report", "developer guide", "HTML report", or "write the docs".
  Read this skill before writing agents/doc_agent.py or prompts/doc_gen.txt.
---

# Documentation Agent Skill

**File:** `agents/doc_agent.py`
**Prompt:** `prompts/doc_gen.txt`
**Reads:** all three state files + manifest
**Writes:** `output/docs/` (5 documents)
...
|----------|------|----------|
| Migration Report | `output/docs/migration_report.md` | File-by-file mapping, framework decisions, dependency translation |
| Validation Report | `output/docs/validation_report.md` | Equivalence score, all mismatches, fixes |
| Test Report | `output/docs/test_report.md` | Test count, coverage %, test case list |
| Developer Guide | `output/docs/developer_guide.md` | How to run, extend, maintain the Python project |
| Full HTML Report | `output/docs/full_report.html` | All 4 docs combined into one page |
...
class DocAgent:
    ...
    def _gen_migration_report(self) -> str:
        ...
        (self.docs_dir / "migration_report.md").write_text(md)
        return md
    ...
    def _gen_validation_report(self) -> str:
        ...
        (self.docs_dir / "validation_report.md").write_text(md)
        return md
    ...
    def _gen_test_report(self) -> str:
        ...
        (self.docs_dir / "test_report.md").write_text(md)
        return md
    ...
    def _gen_developer_guide(self) -> str:
        system = self.pb.build("doc_gen", {
            ...
            "section_data": json.dumps({
                "repo_url":    self.manifest["repo_url"],
                "python_dir":  "output/python_project/",
        ...
        (self.docs_dir / "developer_guide.md").write_text(md)
        return md
    ...
    def _gen_html_report(self, migration_md, validation_md, test_md, devguide_md):
        ...
        try:
            ...
            # Fallback: wrap each section in <pre>
...
## Documentation Prompt Template (`prompts/doc_gen.txt`)

```
You are a technical writer producing migration documentation for developers.
...
- Highlight anything that needs manual review in a > blockquote
- Keep each section under 600 words
...
Store this in `agents/doc_agent.py` as a module-level constant:
...
> **Note for AI builder:** The `{status_class}` variable should be set to
> `"pass"`, `"review"`, or `"fail"` based on the validation status string
...
```
markdown>=3.6    # for HTML conversion; fallback to <pre> if not installed
```
